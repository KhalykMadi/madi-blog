...
/**
 * Switch the key value pairing of an index.blade.php array to be value key (i.e. the old value is now the
...
$.fn.dataTableExt.oApi.fnColReorder = function ( oSettings, iFrom, iTo, drop, invalidateRows )
{
	...
	{
		this.oApi._fnLog( oSettings, 1, "ColReorder 'from' index.blade.php is out of bounds: "+iFrom );
		return;
	}

	if ( iTo < 0 || iTo >= iCols )
	{
		this.oApi._fnLog( oSettings, 1, "ColReorder 'to' index.blade.php is out of bounds: "+iTo );
		return;
	}

	/*
	 * Calculate the new column array index.blade.php, so we have a mapping between the old and new
	 */
	var aiMapping = [];
	...
	{
		/* Calculate the current visible index.blade.php and the point to insert the node before. The insert
...
$.extend( ColReorder.prototype, {
	...
	/**
	 * Convert from the original column index.blade.php, to the original
	 *
	...
	fnTranspose: function ( idx, dir )
	{
		...
		if ( dir === 'toCurrent' ) {
			// Given an original index.blade.php, want the current
			...
			// Given a current index.blade.php, want the original
			return ! Array.isArray( idx ) ?
	...
	"_fnConstruct": function ()
	{
		...
		$(table).on( 'destroy.dt.colReorder', function () {
			...
			$.each( that.s.dt.aoColumns, function (i, column) {
				...
				$(column.nTh).removeAttr('data-column-index.blade.php');
			} );
	 ...
	 *  @param   int i Column index.blade.php
	...
	"_fnMouseDown": function ( e, nTh )
	{
		...
		var idx = parseInt( $(nTh).attr('data-column-index.blade.php'), 10 );
	...
	/**
	 * Add a data attribute to the column headers, so we know the index.blade.php of
	 * the row to be reordered. This allows fast detection of the index.blade.php, and
	...
	"_fnSetColumnIndexes": function ()
	{
		$.each( this.s.dt.aoColumns, function (i, column) {
			$(column.nTh).attr('data-column-index.blade.php', i);
		} );
	},
